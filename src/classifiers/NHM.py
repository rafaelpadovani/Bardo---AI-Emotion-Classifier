...
logs = codecs.open('testesNSK/LogEp'+str(episodioTeste)+'.txt', 'w', 'utf-8')
...
for episodio in range(1, (numepisodios+1)):
    ...
    ##                janelaArray.append(janela)
                ...
                print(str(rpg_ec.calculate_accuracy(confusion_matrix)), 'Janela: ', str(janela), 'K: ', str(kvalue), 'DifTransicao: ', str(DifTransicao))


                logs.write(str(resultado)+':')
                logs.write(str(janela)+':')
                logs.write(str(kvalue)+':')
